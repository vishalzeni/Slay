...
const BannerList = () => {
  ...
  const API_KEY = "mansisumansi"; // Your provided API key

  const fetchBanners = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://localhost:5000/api/banner/banners", {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editingBanner
      ? `http://localhost:5000/api/banner/admin/update/${editingBanner._id}`
      : `http://localhost:5000/api/banner/admin/create`;
  ...
  const toggleStatus = async (id) => {
    try {
      const res = await fetch(`http://localhost:5000/api/banner/admin/toggle/${id}`, {
        method: "PATCH",
        headers: { "x-api-key": API_KEY },
      });
      if (res.ok) {
  ...
  const deleteBanner = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/banner/admin/delete/${id}`, {
        method: "DELETE",
        headers: { "x-api-key": API_KEY },
      });
      if (res.ok) {
